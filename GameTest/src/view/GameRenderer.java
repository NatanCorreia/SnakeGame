...
public class GameRenderer {
	...
	public void drawGame(GameFrame frame, Graphics g) {
		int cellSize = Math.min(panelSize.width / frame.getMap().getQtdCellsWidth(),
				panelSize.height / frame.getMap().getQtdCellsHeight());
		...
		g.fillOval(cellSize * frame.getGoal().getLocation().x, cellSize * frame.getGoal().getLocation().y, cellSize,
				cellSize);
		...
		for (int i = 0; i < snekLenght; i++) {
			...
			g.fillRect(cellSize * currentSegment.getLocation().x, cellSize * currentSegment.getLocation().y, cellSize,
					cellSize);
			g.setColor(Color.green);
		}
		drawScore(g,frame);
		if(frame.getDifficulty()==2) {
		for (int i = 0; i < frame.getMap().getObstacles().getSegments().size(); i++) {
			g.setColor(frame.getMap().getObstacles().getColor());
			g.fillRect((cellSize * frame.getMap().getObstacles().getSegments().get(i).getLocation().x),
	...
	public void drawScore(Graphics g,GameFrame frame) {
		g.setColor(Color.red);
		g.setFont(new Font("Ink Free", Font.BOLD, 40));
		FontMetrics metrics = g.getFontMetrics(g.getFont());
		g.drawString("SCORE: " + frame.getScore(),
				(panelSize.width - metrics.stringWidth("SCORE: " + frame.getScore())) / 2,
				panelSize.height - g.getFont().getSize());
	}
